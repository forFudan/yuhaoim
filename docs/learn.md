...
>例如：
>
>- `女`，第一筆爲「折」，故而它的大碼在折區的`C`。「女」的頻率很高，所以小碼是v。因此，「女」這個字根的編碼就是`Cv`。
...
字根字有大碼和小碼，那麽想要輸入字根該怎麽做呢？很簡單，就是添上`F`：

- 代表字根：大碼 + 小碼 + F
- 被歸併到代表字根上的附屬根、近似根、部首、兼容字：大碼 + 小碼 + FF

{: .example }
>比如：
>
>- `月` 的大碼是`W`，小碼是`v`，它是一個代表字根，所以輸入`Wvf`就可以打出這個字。
>- `勹` 的大碼是`W`，小碼是`v`，它被歸併到了`月`上，所以輸入`Wvff`就可以打出這個部首。
>- `⺼` 的大碼是`W`，小碼是`v`，它被歸併到了`月`上，所以輸入`Wvff`就可以打出這個部首。
...
>- 「其」這個字根，在字母`D`的方框裏，同時，右下角的字母是`q`，因此，它的編碼就是`Dq`。

字根圖是查詢字根的重要工具，初學者應當時常將它放在手邊，以利用查詢。以下爲宇浩輸入法全部字根的鍵位圖。繁體特有字根22個，都排在簡體字根後。

![宇浩輸入法宋體字根圖](../image/宇浩輸入法宋體字根圖.png)
...
>比如：
>
>- 「蘭」可以拆成「䒑二」也可以拆成「丷三」。觀察到，上面的兩點同橫相連，故而拆成「䒑二」符合「能散不連」。
...
>- 「燻」拆作「千黑」，不拆成「千囗丷二灬」。雖然前者將中間的豎斷開了，但因爲「字根最少」的優先級大於「筆畫斷開」，我們選擇只有兩根的「千黑」。
...
>比如：
>
>- 「單」可以拆成「丷日十」也可以拆成「丷冂豐」。兩者都是三根，都完全「符合筆順」，由於出現了「囗」結構，故而跳過「散連交斷」規則，直接判斷「拆分美觀」。
...
單字的編碼一般是由**4個字母**構成，部分情况下是2個或3個字母。
...
>舉例説明：
>
>- 「嫩」字拆成`女Cv 木Fv 口Lv 攵Eh`四根。分别對應了首根、二根、三根、末根。我們直接取四個字根的大碼`CFLE`卽可出字。
>- 「整」字拆成`木Fv 口Lv 攵Eh 一Fi 止Ni`五根。我們只取首根、二根、三根、末根，也就是`木Fv 口Lv 攵Eh 止Ni`的大碼，輸入`FLEN`卽可出字。
>- 「算」字拆成`𥫗Qv 目Mu 廾So`，只有三根，所以我們取全部根，也就是`𥫗Qv 目Mv 廾So`的大碼，卽`QMS`。此時，注意到不足四碼，故而補上最末根的小碼`o`。輸入`QMSo`卽可出字。
>- 「織」字拆成`糹Vs 口Lv 八Tb`，只有三根，所以我們取全部根大碼，也就是`VLT`。此時，注意到不足四碼，故而補上最末根的小碼`b`。輸入`VLTb`卽可出字。
>- 「認」字拆成`訁人`，只有兩根，所以我們取全部根，也就是`訁Oa 人Te`的大碼，卽`OT`。此時，注意到不足四碼，故而補上最末根的小碼`e`。注意到仍然不足四碼，於是再添上首根的小碼`a`。輸入`OTea`卽可出字。
>- 「好」字拆成`女子`，只有兩根，所以我們取全部根，也就是`女Cv 子Vk`的大碼，卽`CV`。此時，注意到不足四碼，故而補上最末根的小碼`k`。注意到仍然不足四碼，但首根小碼爲`v`，可以省略，所以最後輸入`CVk`卽可出字。

末尾添加首根的小碼，只是爲了補齊四碼的作用，只有在雙根字中纔會出現。它不需要像徐碼和鄭碼一樣，不管幾碼字，都要判斷首根的性質以決定取不取倒數第二根。所以思維負擔顯著降低。

{: .note }
>Q：爲什麽選擇 `v` 鍵？  
>A：漢語拼音中没有 `v` 字母，因此這個按鍵使用率不高。但他位於大拇指上，按起來非常方便，所以某些字根的小碼設置成了 `v`。由於每個大碼上基本都有個小碼是 `v` 的字根，因此少打 `v` 節約的碼長最爲明顯。

### 字根字和代表根
<!-- {: .no_toc } -->

字根字，依次取其大小碼，再加上`f`卽可出字。
...
如果要**單獨**輸入「被代表根」，只要在大小碼後加`ff`卽可。這類漢字並不多。

{: .example }
>舉例：
>
>- `羽Xu`、`習Xu`被歸爲一個字根，由「羽」統攝，所以輸入「習」字時，編碼爲`Xuff`。
>- `長Ha`、`镸Ha`、`髟Ha`被歸爲一個字根，由「長」統攝，所以輸入「镸」、「髟」時，編碼爲`Haff`。
>- `户Ih`、`户Ih`被歸爲一個字根，由「户」統攝，所以輸入「户」時，編碼爲`Ihff`。
...
- 一個字的編碼等於4位，且有後續字符等待輸入。我們不需要按空格鍵。只要直接輸入下一個字的首碼，這個字就會自動上屏。我們稱之爲「五碼頂屏」。例如：我們打「霽雨」二字，「霽」字的編碼是`〔雨文{介下同八}・DPTb〕`。輸入`b`後，我們直接輸入「雨」字的編碼`〔雨・Dyƒ〕`，則「霽」字會自動上屏。
...
>- 「事情」：「事」的編碼爲`〔{於下}口彐・ALXv〕`，「情」的編碼爲`〔心龶月・YHWv〕`。因此，「事情」的編碼爲`〔{於下}口心龶・ALYH〕`。
...
>- 「星期三」：「星」的編碼爲`〔日生・JWg〕`，「期」的編碼爲`〔其月・DWvq〕`，「三」的編碼爲`〔三・Fa〕`。因此，「星期三」的編碼爲`〔日其三・JDFa〕`。
...
>比如：
>
>- 「狐假虎威」：「狐」的編碼爲`〔犭瓜・RQan〕`，「假」的編碼爲`〔亻屍二又・EBSB〕`，「虎」的編碼爲`〔虎・Mh〕`，「威」的編碼爲`〔戊一女・DFCv〕`。因此，「狐假虎威」的編碼爲`〔犭亻虎戊・REMD〕`。
...
「旬」的外框「勹」，同「敖」左下的「𠂊」都是撇加橫折鉤。宇浩輸入法不做區分，都視爲「月」的附屬根`Wv`。
